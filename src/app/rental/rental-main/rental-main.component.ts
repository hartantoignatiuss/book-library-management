...
export class RentalMainComponent {
  ...
  ngOnInit() {
    this.getAllRentalsData();
  }

  getAllRentalsData() {
    forkJoin([
      this.BooksService.getBooks(),
      this.RentalService.getMembers(),
    ]).subscribe(([responseBooks, responseMembers]) => {
      ...
      this.RentalService.getRentals().subscribe(
        (rental: Rental[]) => {
          ...
          for (let i = 0; i < loop; i++) {
            ...
            rental[i].rentalLabel =
              this.getMemberData(rental[i].memberId).name +
              ' - ' +
              this.book.name;
          }
          this.rentals = rental;
